        ...
        -- YENİ: Potion Market Kordinatları
        local potionCoords = Vector3.new(-11.61, 10960.55, 221.98)
            ...
            for _, obj in pairs(Workspace:GetChildren()) do
                ...
                if obj.Name == "Island" then
                    for _, child in pairs(obj:GetChildren()) do
                        if not child.Name:lower():find("candy") then
        ...
        -- ADIM 0: Oyunun ve Network'ün yüklenmesini bekle
        if not game:IsLoaded() then game.Loaded:Wait() end
        task.wait(2) -- Başlangıç için ekstra güvenlik beklemesi

        -- ADIM 1: Adaya Işınlan (Pazar yerinin yüklenmesi için önce buraya gitmeliyiz)
        ...
        if tpRemote then
            ...
            -- ADIM 1.5: Potion Market'e Özel Işınlan (Dükkanın etrafında yüklenmesi için)
            if lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
                print("🧪 Potion Market kordinatlarına gidiliyor...")
                lp.Character:PivotTo(CFrame.new(potionCoords) * CFrame.new(0, 3, 0))
                task.wait(3) -- Dükkan ve Remote'ların yüklenmesi için tam süre tanı
            end
        else
        ...
        if eggModel then
            ...
            -- ADIM 4: Auto Potion (Otomatik İksir Satın Alma)
            task.spawn(function()
                print("🧪 Auto Potion Döngüsü Başlatıldı!")
                while _G.AutoPotion do
                    -- Hem PotionMachine hem de BuyPotionMachine isimlerini dene
                    local potionRemote = getGameRemote("BuyPotionMachine") or getGameRemote("PotionMachine")
                    if potionRemote then
                        pcall(function()
                            -- 1. argüman genelde iksir ID'si veya tipi olur
                            potionRemote:InvokeServer(1)
                        end)
                    end
                    task.wait(2) -- Daha stabil bir bekleme (2sn)
                end
            end)
